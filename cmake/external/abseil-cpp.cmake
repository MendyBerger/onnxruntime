...
if(CMAKE_SYSTEM_NAME STREQUAL "WASI")
  ...
  # Prevent building synchronization libraries for WASI
  set(ABSL_BUILD_DLL OFF)
  # Set compile option to disable Abseil features that require synchronization
  add_compile_definitions(ABSL_INTERNAL_DISABLE_WAITER=1)
endif()
...
# For WASI: Replace synchronization targets with empty dummy targets
if(CMAKE_SYSTEM_NAME STREQUAL "WASI" AND abseil_cpp_SOURCE_DIR)
  message(STATUS "Replacing Abseil synchronization targets with empty dummies for WASI")

  # After Abseil is configured, replace synchronization targets
  # We need to do this after the targets are created but before they're used

  # Create empty source file for dummy targets
  file(WRITE "${CMAKE_CURRENT_BINARY_DIR}/abseil_dummy.cc" "// Empty file for WASI dummy targets\n")

  # Get all Abseil targets
  get_directory_property(all_targets DIRECTORY ${abseil_cpp_SOURCE_DIR} BUILDSYSTEM_TARGETS)

  foreach(target ${all_targets})
    if(target MATCHES "absl_(synchronization|graphcycles_internal|kernel_timeout_internal)")
      message(STATUS "  Replacing target with dummy: ${target}")

      # Get the target type
      get_target_property(target_type ${target} TYPE)

      if(target_type STREQUAL "STATIC_LIBRARY" OR target_type STREQUAL "OBJECT_LIBRARY")
        # Clear the sources and add our empty dummy
        set_target_properties(${target} PROPERTIES
          SOURCES "${CMAKE_CURRENT_BINARY_DIR}/abseil_dummy.cc"
          LINK_LIBRARIES ""
...
# For WASI, we skip synchronization-related libraries
if(CMAKE_SYSTEM_NAME STREQUAL "WASI")
  set(ABSEIL_LIBS
  absl::absl_log
  ...
  # Non-WASI builds include synchronization
  set(ABSEIL_LIBS
  absl::absl_log
  absl::log_internal_log_impl
  absl::log_internal_strip
