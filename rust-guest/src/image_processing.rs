...
// use crate::cosmonic::onnx_runtime::types::TensorValueType;
...
// impl TryFrom<ModelImage> for crate::cosmonic::onnx_runtime::types::Value<TensorValueType<f32>> {
impl TryFrom<ModelImage> for crate::cosmonic::onnx_runtime::types::Tensor {
    type Error = Error;
    ...
    fn try_from(ModelImage(size, variant, img): ModelImage) -> Result<Self, Self::Error> {
        ...
        // let data: Vec<u8> = safe_transmute::transmute_vec(
        //     array.as_slice().unwrap().to_vec()
        // ).unwrap();
        // // let vec_f32: Vec<f32> = array.into_raw_vec();
        // // let data: Vec<u8> = safe_transmute::transmute_vec(vec_f32).unwrap();
        ...
        //     // safe_transmute::transmute_vec(array.as_slice().unwrap().to_vec()).unwrap().as_slice(),
        //     &data,
        //     // &[size as u64, size as u64, 3],
        //     &[1, 3, size as u64, size as u64], // Match the actual array shape [N, C, H, W]
        // ).unwrap())
        ...
        //     let len = vec_f32.len() * std::mem::size_of::<f32>();
        //     let capacity = vec_f32.capacity() * std::mem::size_of::<f32>();
        //     let ptr = vec_f32.as_ptr() as *mut u8;
        //     std::mem::forget(vec_f32);
        //     Vec::from_raw_parts(ptr, len, capacity)
        // };
        // Safely convert Vec<f32> to Vec<u8> by converting each f32 to bytes
