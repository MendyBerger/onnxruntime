...
int mp4_h26x_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int length, unsigned frame_duration_90k);
...
static int mp4_h265_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int sizeof_nal, unsigned frame_duration_90k)
{
    ...
    default:
            ...
            return MP4E_STATUS_BAD_ARGUMENTS;
        {
            ...
            err = MP4E_put_sample(h->mux, h->mux_track_id, tmp, 4 + sizeof_nal, frame_duration_90k, sample_kind);
            free(tmp);
        }
        break;
    }
...
int mp4_h26x_write_nal(mp4_h26x_writer_t *h, const unsigned char *nal, int length, unsigned frame_duration_90k)
{
    ...
    for (;;nal++)
    {
        ...
        if (h->is_hevc)
        {
            ERR(mp4_h265_write_nal(h, nal, sizeof_nal, frame_duration_90k));
            continue;
        }
        ...
        default:
            ...
            if (!h->need_pps && !h->need_idr)
            {
                ...
                if (first_mb_in_slice)
                    ...
                    sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
                err = MP4E_put_sample(h->mux, h->mux_track_id, nal2, sizeof_nal, frame_duration_90k, sample_kind);
            }
            break;
        }
        free(nal1);
