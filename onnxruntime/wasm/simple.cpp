...
std::vector<float> preprocess_rgb(const uint8_t* rgb, int width, int height) {
    // DEBUG: Check input data (FIRST CALL ONLY)
    static bool debug_preprocess_done = false;
    bool is_first_call = !debug_preprocess_done;
    
    ImageUtils::Image img(width, height, 3);
    std::memcpy(img.data.data(), rgb, width * height * 3);
    
    if (is_first_call) {
        std::cout << "       [PREPROCESS DEBUG] Input: " << width << "x" << height 
                  << " RGB[0-5]=" << (int)rgb[0] << "," << (int)rgb[1] << "," << (int)rgb[2]
                  << "," << (int)rgb[3] << "," << (int)rgb[4] << "," << (int)rgb[5] << std::endl;
        
        // Save the input before resize
        ImageUtils::Image debug_input(width, height, 3);
        debug_input.data.assign(rgb, rgb + width * height * 3);
        ImageUtils::saveImage("/video/debug_input_before_resize.jpg", debug_input);
        std::cout << "       [PREPROCESS DEBUG] Saved debug_input_before_resize.jpg" << std::endl;
    }
    ...
    if (width != 256 || height != 256) {
        ...
        if (is_first_call) {
            std::cout << "       [PREPROCESS DEBUG] After resize, RGB256[0-5]=" 
                      << (int)img.data[0] << "," << (int)img.data[1] << "," << (int)img.data[2]
                      << "," << (int)img.data[3] << "," << (int)img.data[4] << "," << (int)img.data[5] << std::endl;
            // Save the resized 256x256 input
            ImageUtils::saveImage("/video/debug_preprocessed_256.jpg", img);
            std::cout << "       [PREPROCESS DEBUG] Saved debug_preprocessed_256.jpg" << std::endl;
            debug_preprocess_done = true;  // Mark as done AFTER all saves
        }
    } else if (is_first_call) {
        debug_preprocess_done = true;
    }
...
std::vector<uint8_t> watermark_frame_rgb(
    ...
    // DEBUG: Save mask output
    static bool debug_mask_saved = false;
    if (!debug_mask_saved) {
        debug_mask_saved = true;
        // Mask is 1x1x256x256, convert to grayscale image
        ImageUtils::Image mask_img(256, 256, 1);
        for (int i = 0; i < 256 * 256; i++) {
            mask_img.data[i] = std::clamp(static_cast<int>(mask_data[i] * 255.0f), 0, 255);
        }
        ImageUtils::saveImage("/video/debug_mask.jpg", mask_img);
        std::cout << "       [DEBUG] Saved mask output: min=" << *std::min_element(mask_data, mask_data + 256*256)
                  << " max=" << *std::max_element(mask_data, mask_data + 256*256) << std::endl;
    }
    ...
    // DEBUG: Save watermark encoder output
    static bool debug_wm_saved = false;
    if (!debug_wm_saved) {
        debug_wm_saved = true;
        auto wm_256 = postprocess_to_rgb(wm_data, 256, 256);
        ImageUtils::Image wm_img(256, 256, 3);
        wm_img.data = wm_256;
        ImageUtils::saveImage("/video/debug_wm_output.jpg", wm_img);
        std::cout << "       [DEBUG] Saved watermark encoder output" << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "\n🔄 Processing frames (this will take a while in WASM)..." << std::endl;
    std::cout << "   Expected: ~2-4 seconds per frame" << std::endl;
    
    int frames_processed = 0;
    int sample_count = 0;  // Track total input samples for timestamp calculation
    ...
    for (unsigned sample_idx = 0; sample_idx < num_samples; sample_idx++) {
        ...
        // DEBUG: Print info and save first successfully decoded frame
        static bool first_decoded_saved = false;
        if (!first_decoded_saved) {
            first_decoded_saved = true;
            std::cout << "       [DEBUG] First decoded frame: width=" << dec_width << " height=" << dec_height 
                      << " y_stride=" << y_stride << " uv_stride=" << uv_stride << std::endl;
            std::cout << "       [DEBUG] Y[0]=" << (int)dst[0][0] << " U[0]=" << (int)dst[1][0] 
                      << " V[0]=" << (int)dst[2][0] << std::endl;
            std::cout << "       [DEBUG] RGB[0]=" << (int)rgb[0] << " RGB[1]=" << (int)rgb[1] 
                      << " RGB[2]=" << (int)rgb[2] << std::endl;
                      
            ImageUtils::Image debug_img(dec_width, dec_height, 3);
            debug_img.data = rgb;
            ImageUtils::saveImage("/video/wasm_frame_decoded.jpg", debug_img);
            std::cout << "       [DEBUG] Saved wasm_frame_decoded.jpg" << std::endl;
        }
        
        // Watermark frame
        auto wm_rgb = watermark_frame_rgb(
            watermark_session, mask_session,
            rgb.data(), dec_width, dec_height,
            message, strength
        );
        
        // DEBUG: Save first watermarked frame
        static bool first_watermarked_saved = false;
        if (!first_watermarked_saved) {
            first_watermarked_saved = true;
            ImageUtils::Image debug_wm(dec_width, dec_height, 3);
            debug_wm.data = wm_rgb;
            ImageUtils::saveImage("/video/wasm_frame_watermarked.jpg", debug_wm);
            std::cout << "       [DEBUG] Saved wasm_frame_watermarked.jpg" << std::endl;
        }
        ...
        rgb_to_yuv420(wm_rgb.data(), y_plane.data(), u_plane.data(), v_plane.data(), dec_width, dec_height);
        ...
        if (sample_idx < 3) {
            std::cout << "       [ENCODE DEBUG] Frame " << sample_idx 
                      << ": encode_result=" << encode_result 
                      << " frame_type=" << frame_info.eFrameType 
                      << " layer_num=" << frame_info.iLayerNum << std::endl;
            
            // Log all NAL units in all layers for debugging
            for (int layer = 0; layer < frame_info.iLayerNum; layer++) {
                SLayerBSInfo* layer_info = &frame_info.sLayerInfo[layer];
                std::cout << "         Layer " << layer << ": " << layer_info->iNalCount << " NALs" << std::endl;
                int offset = 0;
                for (int nal = 0; nal < layer_info->iNalCount; nal++) {
                    int nal_size = layer_info->pNalLengthInByte[nal];
                    uint8_t* nal_data = layer_info->pBsBuf + offset;
                    // Check for start code and get NAL type
                    if (nal_size >= 5 && nal_data[0] == 0 && nal_data[1] == 0 && nal_data[2] == 0 && nal_data[3] == 1) {
                        uint8_t nal_type = nal_data[4] & 0x1F;
                        std::cout << "           NAL " << nal << ": type=" << (int)nal_type << " size=" << nal_size << std::endl;
                    }
                    offset += nal_size;
                }
            }
        ...
        if (encode_result == 0 && frame_info.eFrameType != videoFrameTypeInvalid && frame_info.iLayerNum > 0) {
            ...
            if (frames_processed % 10 == 0 || frames_processed <= 3) {
                std::cout << "       ✓ Frame " << frames_processed << " complete!" << std::endl;
            }
        }
    }
